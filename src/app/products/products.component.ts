...
export class ProductsComponent {
  ...
  addToArr(){
    ...
    // DELETE BY INDEX
  // delet(ind : number){
  //   this.prodArray.splice(ind,1)
  // }
  deletById(id : number){
   let prodId = this.prodArray.findIndex(el => el.id == id)

   if(prodId !== -1){
    this.prodArray.splice(prodId,1)
   }
  }
  ...
  Edit(i : number){
    let findProd = this.prodArray.find(el => el.id == i)

    if(findProd){
      this.products = {...findProd}
    }
  }

  update(){
    let ind = this.prodArray.findIndex(el => el.id == this.products.id)
    if(ind != -1){
      this.prodArray[ind] = {...this.products}
      this.products =  new Product
    }
  }
